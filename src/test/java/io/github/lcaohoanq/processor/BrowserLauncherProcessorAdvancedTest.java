...
class BrowserLauncherProcessorAdvancedTest {
  ...
  void run_shouldHandleClassNotFound() throws Exception {
    ...
    try (MockedStatic<Thread> threadMock = mockStatic(Thread.class)) {
      ...
      try (MockedStatic<Class> classMock = mockStatic(Class.class)) {
        classMock
            .when(() -> Class.forName("com.example.NonExistentClass"))
  ...
  void run_shouldHandleAsyncAnnotation() throws Exception {
    ...
    try (MockedStatic<Thread> threadMock = mockStatic(Thread.class)) {
      ...
      try (MockedStatic<Class> classMock = mockStatic(Class.class)) {
        ...
        try (MockedStatic<JavaBrowserLauncher> launcherMock =
            mockStatic(JavaBrowserLauncher.class)) {
          processor.run(args);

          // Should call async method
          launcherMock.verify(
              () ->
                  JavaBrowserLauncher.doHealthCheckThenOpenHomePageAsync(anyString(), anyString()));
        }
      }
    }
  }
    ...
    try (MockedStatic<Thread> threadMock = mockStatic(Thread.class)) {
      ...
      try (MockedStatic<Class> classMock = mockStatic(Class.class)) {
        ...
        try (MockedStatic<JavaBrowserLauncher> launcherMock =
          ...
          launcherMock.verify(
              () -> JavaBrowserLauncher.doHealthCheckThenOpenHomePage(anyString(), anyString()));
        }
      }
    }
  }
