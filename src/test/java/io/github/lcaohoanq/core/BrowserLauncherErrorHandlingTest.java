...
class BrowserLauncherErrorHandlingTest {
      ...
      assertDoesNotThrow(
          () -> {
            JavaBrowserLauncher.doHealthCheckThenOpenHomePage(
                invalidHealthCheckUrl, "https://example.com");
          });
  ...
  void doHealthCheckThenOpenHomePage_shouldHandleNetworkTimeouts() throws Exception {
    try (MockedStatic<Desktop> desktopMock = mockStatic(Desktop.class)) {
      ...
      assertDoesNotThrow(
          () -> {
            JavaBrowserLauncher.doHealthCheckThenOpenHomePage(
                unreachableUrl, "https://example.com");
          });
      ...
      assertTrue(
          errorOutput.contains("Health check failed") || errorOutput.contains("exception"),
  ...
  void doHealthCheckThenOpenHomePage_shouldHandle4xxErrors() throws Exception {
    ...
    try (MockedStatic<Desktop> desktopMock = mockStatic(Desktop.class)) {
      ...
      JavaBrowserLauncher.doHealthCheckThenOpenHomePage(
          "http://localhost:8085/health", "https://example.com");
      ...
      assertTrue(
          output.contains("Health check failed with status code: 404"), "Should log 404 error");
    }
  }
  ...
  void doHealthCheckThenOpenHomePage_shouldHandle5xxErrors() throws Exception {
    ...
    try (MockedStatic<Desktop> desktopMock = mockStatic(Desktop.class)) {
      ...
      String output = outputStreamCaptor.toString();
      assertTrue(
          output.contains("Health check failed with status code: 500"), "Should log 500 error");
    }
  }
  ...
  void doHealthCheckThenOpenHomePageAsync_shouldHandleNetworkExceptions() throws Exception {
    try (MockedStatic<Desktop> desktopMock = mockStatic(Desktop.class)) {
      ...
      CompletableFuture<Void> future =
          JavaBrowserLauncher.doHealthCheckThenOpenHomePageAsync(
              "http://192.0.2.1:8080/health", "https://example.com");
  ...
  void doHealthCheckThenOpenHomePageAsync_shouldHandleNullHealthCheckUrl() throws Exception {
    try (MockedStatic<Desktop> desktopMock = mockStatic(Desktop.class)) {
      ...
      String output = outputStreamCaptor.toString();
      assertTrue(
          output.contains("Health check endpoint is null or empty"), "Should log skipping message");
    }
  }
}
