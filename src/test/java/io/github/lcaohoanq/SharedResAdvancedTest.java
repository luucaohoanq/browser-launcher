...
class SharedResAdvancedTest {
  ...
  void httpClient_shouldBeStaticFinalField() throws NoSuchFieldException {
    ...
    assertTrue(Modifier.isStatic(httpClientField.getModifiers()), "HTTP_CLIENT should be static");
    assertTrue(Modifier.isFinal(httpClientField.getModifiers()), "HTTP_CLIENT should be final");
    assertTrue(Modifier.isPublic(httpClientField.getModifiers()), "HTTP_CLIENT should be public");
  }
  ...
  void httpClient_shouldBeCorrectType() throws Exception {
    ...
    assertInstanceOf(
        HttpClient.class, SharedRes.HTTP_CLIENT, "HTTP_CLIENT should be instance of HttpClient");
  }
  ...
  void httpClient_shouldBeThreadSafe() throws InterruptedException {
    ...
    for (int i = 1; i < clients.length; i++) {
      assertSame(clients[0], clients[i], "All HTTP_CLIENT instances should be the same");
    }
  }
  ...
  void sharedRes_shouldOnlyContainHttpClient() throws Exception {
    ...
    assertEquals("HTTP_CLIENT", fields[0].getName(), "The only field should be HTTP_CLIENT");
  }
  ...
  void sharedRes_shouldNotHaveConstructor() throws Exception {
    ...
    assertEquals(
        1,
        SharedRes.class.getDeclaredConstructors().length,
  ...
  void sharedRes_shouldNotBeInstantiable() throws Exception {
    ...
    assertTrue(
        SharedRes.class.getDeclaredFields().length > 0, "SharedRes should have static fields");
  }
  ...
  void httpClient_shouldBeImmutableReference() throws NoSuchFieldException {
    ...
    assertTrue(
        Modifier.isFinal(httpClientField.getModifiers()),
